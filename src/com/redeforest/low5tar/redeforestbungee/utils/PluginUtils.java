package com.redeforest.low5tar.redeforestbungee.utils;
...
public final class PluginUtils {
	...
	public static void unloadPlugin(Plugin plugin) {
		...
		Thread.getAllStackTraces().keySet().stream()
				.filter(thread -> (thread.getClass().getClassLoader() == pluginclassloader)).forEach(thread -> {
					try {
						thread.interrupt();
						thread.join(2000);
						if (thread.isAlive()) {
		...
		try (JarFile jar = new JarFile(pluginfile)) {
			...
			try (InputStream in = jar.getInputStream(pdf)) {
				...
				ProxyServer.getInstance().getPluginManager().getPlugins().stream()
						.forEach(plugin -> plugins.add(plugin.getDescription().getName()));
				...
				Map<String, Plugin> pluginsMap = ReflectionUtils
						.getFieldValue(ProxyServer.getInstance().getPluginManager(), "plugins");
